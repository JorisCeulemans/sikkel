...
ext-subst-to-term : {Δ Γ : Ctx C ℓ} {T : Ty Γ} (τ : Δ ⇒ Γ ,, T) → Tm Δ (T [ π ⊚ τ ])
ext-subst-to-term {T = T} τ = ι⁻¹[ ty-subst-comp T π τ ] (ξ [ τ ]')

to-ext-subst : {Δ Γ : Ctx C ℓ} (T : Ty Γ) (σ : Δ ⇒ Γ) → Tm Δ (T [ σ ]) → Δ ⇒ Γ ,, T
...
to-ext-subst-Σ : {Δ Γ : Ctx C ℓ} (T : Ty Γ) → Σ[ σ ∈ Δ ⇒ Γ ] (Tm Δ (T [ σ ])) → Δ ⇒ Γ ,, T
to-ext-subst-Σ T [ σ , t ] = to-ext-subst T σ t
-}

ctx-ext-subst-proj₁ : {Δ Γ : Ctx C ℓ} {T : Ty Γ} (σ : Δ ⇒ Γ) (t : Tm Δ (T [ σ ])) → π ⊚ ⟨ σ , t ∈ T ⟩ ≅ˢ σ
eq (ctx-ext-subst-proj₁ σ t) δ = refl

ctx-ext-subst-proj₂ : {Δ Γ : Ctx C ℓ} {T : Ty Γ} (σ : Δ ⇒ Γ) (t : Tm Δ (T [ σ ])) →
...
ctx-ext-subst-η : {Δ Γ : Ctx C ℓ} {T : Ty Γ} (τ : Δ ⇒ Γ ,, T) → ⟨ π ⊚ τ , ext-subst-to-term τ ∈ T ⟩ ≅ˢ τ
eq (ctx-ext-subst-η τ) δ = refl

ctx-ext-subst-comp : {Δ Γ Θ : Ctx C ℓ} {T : Ty Θ} (σ : Γ ⇒ Θ) (t : Tm Γ (T [ σ ])) (τ : Δ ⇒ Γ) →
