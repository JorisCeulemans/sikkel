...
Stream' : ClosedType ★ fℓ → ClosedType ★ fℓ
...
now-timeless-ctx-nul : {A : ClosedType ★ fℓ} {{_ : IsClosedNatural A}} {Γ : Ctx ★ ℓc} →
                       Tm Γ A → Tm (now (timeless-ctx Γ)) A
now-timeless-ctx-nul t = ι[ by-naturality ] (t [ from now-timeless-ctx ]')

instance
  ⇛-closed : {A : ClosedType ★ fℓ} {{_ : IsClosedNatural A}} {B : ClosedType ★ gℓ} {{_ : IsClosedNatural B}} →
             IsClosedNatural (A ⇛ B)
  closed-natural {{⇛-closed}} σ = by-naturality

map' : {A : ClosedType ★ fℓ} {{_ : IsClosedNatural A}} {B : ClosedType ★ gℓ} {{_ : IsClosedNatural B}} →
       Tm Γ ((A ⇛ B) ⇛ Stream' A ⇛ Stream' B)
map' {A = A}{B = B} =
  lamι[ "f" ∈ A ⇛ B ]
    lamι[ "s" ∈ Stream' A ]
...
module _ {A : ClosedType ★ fℓ} {{_ : IsClosedNatural A}} {Γ : Ctx ω ℓc} where
  g-every2nd : Tm Γ (timeless-ty (Stream' A) ⇛ GStream A)
  g-every2nd = löbι[ "g" ∈▻' (timeless-ty (Stream' A) ⇛ GStream A) ]
                 lamι[ "s" ∈ timeless-ty (Stream' A) ]
                   g-cons $ timeless-tm (head' $ untimeless-tm (varι "s"))
