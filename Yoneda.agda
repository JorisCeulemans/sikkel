...
𝕪[_]_ : ∀ ℓ → Ob → Ctx C (h ⊔ ℓ)
𝕪[ ℓ ] x = 𝕪 {ℓ} x

-- The Yoneda lemma
to-𝕪⇒* : {Γ : Ctx C (h ⊔ ℓ)} {x : Ob} → Γ ⟨ x ⟩ → 𝕪[ ℓ ] x ⇒ Γ
func (to-𝕪⇒* {Γ = Γ} γ) (lift f) = Γ ⟪ f ⟫ γ
naturality (to-𝕪⇒* {Γ = Γ} γ) (lift f) = sym (rel-comp Γ _ f γ)

from-𝕪⇒* : {Γ : Ctx C (h ⊔ ℓ)} {x : Ob} → 𝕪[ ℓ ] x ⇒ Γ → Γ ⟨ x ⟩
from-𝕪⇒* σ = func σ (lift hom-id)

𝕪-to-∘-from : {Γ : Ctx C (h ⊔ ℓ)} {x : Ob} (σ : 𝕪[ ℓ ] x ⇒ Γ) → to-𝕪⇒* (from-𝕪⇒* σ) ≅ˢ σ
eq (𝕪-to-∘-from σ) (lift f) = trans (naturality σ (lift hom-id))
                                    (cong (func σ ∘ lift) hom-idˡ)

𝕪-from-∘-to : {Γ : Ctx C (h ⊔ ℓ)} {x : Ob} (γ : Γ ⟨ x ⟩) → from-𝕪⇒* {ℓ = ℓ} {Γ = Γ} (to-𝕪⇒* γ) ≡ γ
...
𝕪-comp : {Γ : Ctx C (h ⊔ ℓ)} (f : Hom x y) (γ : Γ ⟨ y ⟩) → to-𝕪⇒* {ℓ = ℓ} {Γ = Γ} γ ⊚ to-𝕪⇒𝕪 f ≅ˢ to-𝕪⇒* (Γ ⟪ f ⟫ γ)
