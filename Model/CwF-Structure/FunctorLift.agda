...
module Model.CwF-Structure.FunctorLift {C D : BaseCategory} (F : BaseFunctor C D) where
...
lift-ty : {Γ : Ctx D} → Ty Γ → Ty (lift-ctx Γ)
...
lift-tm : {Γ : Ctx D} {T : Ty Γ} → Tm Γ T → Tm (lift-ctx Γ) (lift-ty T)
...
lift-tm-natural : {Δ : Ctx D} {Γ : Ctx D} (σ : Δ ⇒ Γ) {T : Ty Γ} (t : Tm Γ T) →
                  lift-tm (t [ σ ]') ≅ᵗᵐ ι[ lift-ty-natural σ T ] ((lift-tm t) [ lift-subst σ ]')
...
lift-ctx-ext : (Γ : Ctx D) (T : Ty Γ) → lift-ctx (Γ ,, T) ≅ᶜ lift-ctx Γ ,, lift-ty T
...
lift-π : (Γ : Ctx D) (T : Ty Γ) → lift-subst π ⊚ to (lift-ctx-ext Γ T) ≅ˢ π
...
lift-ξ : (Γ : Ctx D) (T : Ty Γ) → lift-tm ξ [ to (lift-ctx-ext Γ T) ]' ≅ᵗᵐ
                                     ...
                                     ι[ ty-subst-comp (lift-ty T) (lift-subst π) (to (lift-ctx-ext Γ T)) ] (
