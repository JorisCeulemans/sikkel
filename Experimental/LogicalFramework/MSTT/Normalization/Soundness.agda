...
normalize-sound (suc n) (nat-elim z s t) = normalize-sound (suc n) t >>= (λ {x} → normalize-nat-elim-sound n z s t {x})
  where
    ...
    normalize-nat-elim-sound n z s t {neutral net} et = (λ ez → M.nat-elim-cong ez M.reflᵗᵐ et) <$> normalize-sound (suc n) z
    normalize-nat-elim-sound n z s t {zero}        et = (λ ez → M.transᵗᵐ (M.symᵗᵐ (M.β-nat-zero _ _)) (M.nat-elim-cong ez M.reflᵗᵐ et)) <∙> normalize-sound (suc n) z
