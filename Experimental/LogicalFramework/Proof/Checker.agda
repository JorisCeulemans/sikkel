...
is-eq? (t1 ≡ᶠ t2) = ok (is-eq t1 t2)
is-eq? φ = error "Formula is not an equation"
...
check-proof Ξ (trans {T = T'} middle-tm p1 p2) φ = do
  ...
  refl ← T =T? T'
  check-proof Ξ p1 (t ≡ᶠ middle-tm)
  check-proof Ξ p2 (middle-tm ≡ᶠ s)
