...
data Var : Name → CtxExpr → TyExpr → Set where
  vzero : Var x (Γ ,, x ∈ T) T
  vsuc : Var x Γ T → Var x (Γ ,, y ∈ S) T

infixl 50 _∙_
data TmExpr (Γ : CtxExpr) : TyExpr → Set where
  var' : (x : Name) {v : Var x Γ T} → TmExpr Γ T
...
mid-weaken-var : {Γ : CtxExpr} (Δ : CtxExpr) → Var x (Γ ++ctx Δ) T → Var x ((Γ ,, y ∈ S) ++ctx Δ) T
...
subst-var : (v : Var x Γ T) → SubstExpr Δ Γ → TmExpr Δ T
...
var-weaken-subst-trivial-multi : (Θ : CtxExpr) (v : Var x (Γ ++ctx Θ) T) {s : TmExpr Γ S} →
