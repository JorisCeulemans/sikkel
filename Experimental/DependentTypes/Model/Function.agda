...
ty-cong (Pi T S) refl {t = f} = to-pshfun-eq λ _ _ _ → $-cong f refl
ty-id (Pi {Γ = Γ} T S) {t = f} = to-pshfun-eq (λ _ eγ _ → $-cong f hom-idˡ)
ty-comp (Pi {Γ = Γ} T S) {t = f} = to-pshfun-eq (λ _ _ _ → $-cong f ∙assoc)
