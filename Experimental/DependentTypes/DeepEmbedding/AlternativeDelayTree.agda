...
{-# TERMINATING #-}
_t>>=p_ : (m : TCMThunk ℓ′ A) → (∀ v → HasRes (delay m) v → TCM ℓ′ B) → TCMThunk ℓ′ B
_>>=p_ : (m : TCM ℓ′ A) → (∀ v → HasRes m v → TCM ℓ′ B) → TCM ℓ′ B
type-error x >>=p k = type-error x
ok x >>=p k = k x (here refl)
delay x >>=p k = delay (x t>>=p k)
bind m k1 >>=p k2 = m >>=p λ v p → k1 v >>=p λ v′ p′ → k2 v′ (bind p p′)
TCMThunk.force (m t>>=p k) = TCMThunk.force m >>=p (λ v p → k v (next p))
...
HasRes-invert->>=p (delay m) (next hr) = HasRes-invert-t>>=p m hr
...
TCMThunk.force (ty-eq?-thunk Nat Nat Γ sΓ Γ-ok .Nat' .Nat' (next (here refl)) (next (here refl))) = return reflᵗʸ
TCMThunk.force (ty-eq?-thunk Bool Bool Γ sΓ Γ-ok .Bool' .Bool' (next (here refl)) (next (here refl))) = return reflᵗʸ
