...
module Experimental.DependentTypes.DeepEmbedding.SoundTypeChecker (mt : ModeTheory) where
...
open import Experimental.DependentTypes.DeepEmbedding.Syntax mt
...
record InferInterpretResult {m} (Γ : CtxExpr m) : Set
⟦_⟧ctx : CtxExpr m → TCM (Ctx ⟦ m ⟧mode)
⟦_⟧ty-in_ : TyExpr m → (Γ : CtxExpr m) → TCM (tcm-elim (λ _ → ⊤) Ty ⟦ Γ ⟧ctx)
infer-interpret : TmExpr m → (Γ : CtxExpr m) → TCM (InferInterpretResult Γ)


record InferInterpretResult {m} Γ where
  field
    T : TyExpr m
    interpretation : tcm-elim (λ _ → ⊤) (λ ⟦Γ⟧ → tcm-elim (λ _ → ⊤) (λ ⟦T⟧ → {!!}) (⟦ T ⟧ty-in Γ)) ⟦ Γ ⟧ctx
